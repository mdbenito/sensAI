...
class FeatureGenerator(ToStringMixin, ABC):
    ...
    def generate(self, df: pd.DataFrame, ctx=None) -> pd.DataFrame:
        ...
        log.debug("Generating features with %s", self)
    ...
    def flattened(self,
            columns_to_flatten: List[Union[str, ColumnMatcher]] = None,
            normalisation_rules=(),
...
class FeatureGeneratorTakeColumns(RuleBasedFeatureGenerator):
    def __init__(self,
            columns: Union[str, ColumnMatcher, List[Union[str, ColumnMatcher]]] = None,
            except_columns: Sequence[Union[str, ColumnMatcher]] = (),
            categorical_feature_names: Optional[Union[Sequence[str], str]] = (),
            normalisation_rules: Sequence[data_transformation.DFTNormalisation.Rule] = (),
            normalisation_rule_template: data_transformation.DFTNormalisation.RuleTemplate = None,
            verify_column_names=True):
        """
        :param columns: the column(s) to be taken. If None, all columns will be taken.
        :param except_columns: columns to not take if present in the input data frame
        :param categorical_feature_names: either a sequence of column names or a regex that is to match all categorical feature names
            (which must not only work for the feature generated by this feature generator, i.e. it should not match feature names generated
            by other feature generators).
            It will be ensured that the respective columns in the generated data frames will have dtype 'category'.
        ...
        :param verify_column_names: if True and columns to take were specified, will raise an error in case said columns
            are missing during feature generation
        """
        super().__init__(categorical_feature_names=categorical_feature_names, normalisation_rules=normalisation_rules,
            normalisation_rule_template=normalisation_rule_template)
        if isinstance(columns, str) or isinstance(columns, ColumnMatcher):
            columns = [columns]
    ...
    def _generate(self, df: pd.DataFrame, ctx=None) -> pd.DataFrame:
        if self.columns is not None:
            columns_to_take = ColumnMatcherCollection(self.columns) \
                .matching_columns(df.columns, require_all_matchers_applied=self.verifyColumnNames)
        else:
            columns_to_take = df.columns
        if len(self.exceptColumns) > 0:
            columns_to_take = ColumnMatcherCollection(self.exceptColumns).not_matching_columns(columns_to_take)
...
class FeatureGeneratorFlattenColumns(RuleBasedFeatureGenerator):
    """
    Instances of this class take columns with vectors and create a data frame with columns containing entries of
    these vectors.
    ...
    """
    def __init__(self, columns: Optional[Union[str, ColumnMatcher, Sequence[Union[str, ColumnMatcher]]]] = None,
            categorical_feature_names: Sequence[str] = (),
            normalisation_rules: Sequence[data_transformation.DFTNormalisation.Rule] = (),
            normalisation_rule_template: data_transformation.DFTNormalisation.RuleTemplate = None):
        """
        :param columns: the column(s) to be flattened. If None, all columns will be flattened.
    ...
    def _generate(self, df: pd.DataFrame, ctx=None) -> pd.DataFrame:
        # determine columns to be flattened
        if self.columns is None:
            columns_to_flatten = df.columns
        else:
            columns_to_flatten = ColumnMatcherCollection(self.columns).matching_columns(df.columns, require_all_matchers_applied=True)

        # compute new column names and collect values
        all_new_columns = []
        all_values = []
        for col in columns_to_flatten:
            log.debug("Flattening column %s", col)
            ...
            log.debug("Flattening resulted in %d new columns", len(new_columns))

        # create the data frame
        if all_values:
            combined_values = np.concatenate(all_values, axis=1)
            result_df = pd.DataFrame(combined_values, columns=all_new_columns, index=df.index)
        else:
...
def flattened_feature_generator(fgen: FeatureGenerator, columns_to_flatten: List[Union[str, ColumnMatcher]] = None, keep_other_columns=True,
    ...
    :param columns_to_flatten: the output columns to be flattened; if None, flatten all columns
