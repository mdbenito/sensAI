...
class DFTFromFeatureGenerator(DataFrameTransformer):
    ...
    or the original data frame extended with the features data frame
    """
    def __init__(self, fgen: "FeatureGenerator", append: bool = False, copy: bool = True):
        """
        :param fgen: the feature generator from which to generate
        :param append: whether to append the columns generated by the feature generator to the input data frame; if False, the transformed
            data frame will consist only of the generated features data frame
        :param copy: whether, for the case where append=True, the returned data frame shall copy the data of the input
            data frame (rather than reuse the data)
        """
        super().__init__()
        self.fgen = fgen
        self.append = append
    ...
    def __setstate__(self, state):
        setstate(DFTFromFeatureGenerator, self, state, new_default_properties={"append": False, "copy": False})

    def is_fitted(self):
        return self.fgen.is_fitted()

    def _fit(self, df: pd.DataFrame):
        self.fgen.fit(df, ctx=None)

    def _apply(self, df: pd.DataFrame) -> pd.DataFrame:
        feature_df = self.fgen.generate(df)
        if not self.append:
            return feature_df
        else:
            return pd.concat([df, feature_df], axis=1, copy=self.copy)
